<div wire:poll.keep-alive.5s class="p-0 lg:pl-4 lg:py-4 w-full">
    ...
    <div class="hidden md:block">
        <div class="flex space-x-5 text-sm font-medium border-b mb-5">
            <button type="button" wire:click="filter('')"
                class="relative py-2 text-dark-tertiary  hover:text-primary group @if ($selectedOrderStatus == '') {{ 'text-primary' }} @endif">
                Semua
                <span
                    class="absolute left-0 bottom-0 h-0.5 w-full rounded-full bg-primary @if ($selectedOrderStatus == '') {{ 'scale-x-100' }} @else {{ 'scale-x-0 group-hover:scale-x-100' }} @endif transition-transform origin-left"></span>
            </button>
            @foreach ($order_status as $status)
                <button type="button" wire:click='filter({{ $status->id }})'
                    class="relative py-2 text-dark-tertiary hover:text-primary group @if ($selectedOrderStatus == $status->id) {{ 'text-primary' }} @endif">
                    {{ $status->name }}
                    <span
                        class="absolute left-0 bottom-0 h-0.5 w-full rounded-full bg-primary @if ($selectedOrderStatus == $status->id) {{ 'scale-x-100' }} @else {{ 'scale-x-0 group-hover:scale-x-100' }} @endif transition-transform origin-left"></span>
                </button>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 antialiased relative shadow-sm sm:rounded-lg">
        <div class="mb-4 flex justify-between gap-5 items-center">
            <div class="w-full">
                ...
                <div class="relative mt-1">
                    ...
                    <input type="text" id="table-search" wire:model.live='search'
                        ...
                        placeholder="Cari pesanan berdasarkan no pesanan">
                </div>
            </div>
            <button type="button"
            ...
            <!-- Dropdown menu -->
            <div class="z-50 hidden text-base w-50 list-none" id="filter-dropdown">
                <div class="bg-white divide-y divide-gray-100 rounded-lg shadow-lg">
                    <ul class="py-2 flex flex-col" aria-labelledby="filter-menu-button">
                        <li>
                            <button type="button" wire:click="filter('')"
                                class="block px-4 py-1 text-sm font-medium w-full text-start text-dark-primary @if ($selectedOrderStatus == '') {{ 'text-primary' }} @endif hover:bg-gray-100">Semua</button>
                        </li>
                        @foreach ($order_status as $status)
                            <li>
                                <button type="button" wire:click="filter({{ $status->id }})"
                                    class="block px-4 py-1 text-sm font-medium w-full text-start text-dark-primary @if ($selectedOrderStatus == $status->id) {{ 'text-primary' }} @endif hover:bg-gray-100">{{ $status->name }}</button>
                            </li>
                        @endforeach
                    </ul>
                </div>
        ...
        <div class="block md:hidden">
            @if (count($orders) > 0)
                @foreach ($orders as $order)
                    <div class="flex gap-5 border-b pt-3 pb-5" x-data="{ goToDetail: '{{ route('my-account.orders.detail', $order->cart_id) }}' }"
                        @click="window.location.href = goToDetail">
                        <div class="text-sm sm:text-base inline-flex mt-[2px] justify-center w-12 text-dark-secondary">
                            {{ $loop->iteration }}
                        </div>
                        <div class="flex flex-col justify-between w-full">
                            <h3 class="text-sm sm:text-base text-dark-primary">No Pesanan:
                                <span class="font-semibold">NP{{ $order->no_order }}</span>
                            </h3>
                            ...
                            <div
                                ...
                                <p class="text-sm sm:text-base text-dark-primary">Total
                                    {{ count($order->cart->cart_details) }} item: <span
                                        class="text-primary font-medium"><span
                                            class="text-sm">Rp</span>{{ number_format($order->total_price, 0, '', '.') }}</span>
                                </p>
                                {{-- <input type="hidden" name="cart_id" value="{{ $order->cart_id }}" required> --}}
                                ...
                                @else
                                    <a href="{{ route('my-account.place-order', $order->cart_id) }}"
                ...
                @endforeach
            @else
                <div class="justify-center flex-col items-center gap-3 mt-12 flex md:hidden">
                    <img src="{{ asset('images/icons/no-order.png') }}" alt="empty-cart" class="w-16 lg:w-20">
                    <h1 class="text-center text-base xl:text-lg font-semibold text-dark-primary">Belum ada pesanan</h1>
                </div>
            @endif
        </div>
        ...
        <table
                    ...
                    @foreach ($orders as $order)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="flex items-center px-6 py-4">
                                <a href="{{ route('my-account.orders.detail', $order->cart_id) }}"
            ...
            @else
                <div class="justify-center flex-col items-center gap-3 mt-12 hidden md:flex">
                    <img src="{{ asset('images/icons/no-order.png') }}" alt="empty-cart" class="w-16 lg:w-20">
                    <h1 class="text-center text-base xl:text-lg font-semibold text-dark-primary">Belum ada pesanan</h1>
                </div>
            @endif
