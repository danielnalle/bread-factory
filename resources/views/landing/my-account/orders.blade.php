                            ...
                            <div class="bg-white divide-y divide-gray-100 rounded-lg shadow-lg">
                                <ul class="py-2" aria-labelledby="filter-menu-button">
                                    ...
                                    @foreach ($order_status as $status)
                                        <li>
                                            <a href="#"
                                                class="block px-4 py-1 text-sm font-medium text-dark-primary hover:bg-gray-100">{{ $status->name }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                    ...
                    <div class="block md:hidden">
                        ...
                        @foreach ($orders as $order)
                            @if ($order->paymentStatus->id == 3)
                                <div class="flex gap-5 border-b pt-3 pb-5" x-data="{ goToDetail: '{{ route('my-account.orders.detail', ['cart_id' => $cart_id]) }}' }"
                                    @click="window.location.href = goToDetail">
                                @else
                                    <div class="flex gap-5 border-b pt-3 pb-5" x-data="{ placeOrder: '{{ route('my-account.place-order') }}' }"
                                        @click="window.location.href = placeOrder">
                            @endif
                            <div
                            ...
                            <div class="flex flex-col justify-between w-full">
                                <h3 class="text-sm sm:text-base text-dark-primary">No Pesanan:
                                    <span class="font-semibold">NP{{ $order->created_at->format('Ymd') }}</span>
                                </h3>
                                ...
                                <div
                                    ...
                                    <p class="text-sm sm:text-base text-dark-primary">Total
                                        {{ $order->cart->cartDetails->count() }} item: <span
                                    ...
                                    @else
                                        <a href="{{ route('my-account.place-order') }}"
                        ...
                        @foreach ($orders as $order)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <th scope="row"
                                    ...
                                    NP{{ $order->created_at->format('Ymd') }}
                                </th>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs">Rp</span>{{ number_format($order->total_price, 0, '', '.') }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="flex items-center px-6 py-4">
                                    @if ($order->paymentStatus->id == 3)
                                        <a href="{{ route('my-account.orders.detail', ['cart_id' => $cart_id]) }}"
