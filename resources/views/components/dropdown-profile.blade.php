...
<div class="relative inline-flex" x-data="{ open: false }">
    <button class="inline-flex justify-center items-center group" aria-haspopup="true" @click.prevent="open = !open"
        :aria-expanded="open">
        <img class="w-8 h-8 rounded-full border" src="{{ asset('images/user-36-05.jpg') }}" width="32" height="32"
            alt="Yogi Pradnyana" />
    ...
    <div class="origin-top-right z-10 absolute top-full min-w-44 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1 {{ $align === 'right' ? 'right-0' : 'left-0' }}"
        ...
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-gray-200 dark:border-gray-700/60">
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 italic capitalize">{{ auth()->user()->role }}</div>
        </div>
        <ul>
            <li>
                <a class="font-medium text-sm text-primary hover:text-primary-hover dark:hover:text-primary flex items-center py-1 px-3"
                    href="{{ route('landing-page') }}" @click="open = false" @focus="open = true"
                    @focusout="open = false">Landing Page</a>
            </li>
            <li>
                <form action="{{ route('logout') }}" method="POST" class="w-full">
                    @csrf
                    <button type="submit" @focus="open = true" @focusout="open = false"
                        class="font-medium text-sm text-primary hover:text-primary-hover dark:hover:text-primary flex items-center py-1 px-3">Logout</button>
                </form>
            </li>
        </ul>
    </div>
