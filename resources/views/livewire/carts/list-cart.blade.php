<div id="cart" class="px-5 md:px-24 xl:px-44 w-full pt-[84px] pb-10 bg-[#F5F5F5]" x-data="stickyHandler"
    wire:submit='proceedToCheckout'>
    <div class="w-full mb-6">
        ...
        <div class="hidden lg:block bg-white px-5 py-6 shadow-sm rounded-lg mb-5">
            ...
            <div class="grid grid-cols-[repeat(6,_1fr)] gap-5 text-sm sm:text-base font-normal text-dark-primary mt-5">
                <div class="col-span-2 ps-4">Roti</div>
        ...
        <div class="flex flex-col bg-white rounded-lg overflow-hidden shadow-sm">
                        ...
                        <div class="flex gap-5 py-3 sm:py-5">
                            ...
                            <div class="cols-span-2 flex flex-col justify-between w-full">
                                <div class="flex justify-between">
                                    ...
                                    <button type="button" wire:click='removeItem({{ $cart->breads->id }})'
                                ...
                                <div class="flex justify-between items-center gap-2 mt-4 flex-wrap">
                                    ...
                                    <div class="flex h-6 lg:h-8">
                                        <button
                                            ...
                                            class="inline-flex items-center justify-center p-1 w-6 lg:w-8 h-6 lg:h-8 border-t border-l border-b border-dark-tertiary"
                                            {{ $quantities[$cart->id] <= $cart->breads->min_order ? 'disabled' : '' }}>
                                            <svg class="w-2 lg:w-3" height="2" viewBox="0 0 12 2" fill="none"
            ...
            @foreach ($carts as $cart)
                <div class="hidden lg:block px-5">
                    <div
                        class="border-b @if (isset($errorsPerBread[$cart->id])) {{ 'pt-5 pb-2' }} @else {{ 'py-5' }} @endif">
                        <div class="grid grid-cols-[repeat(6,_1fr)] gap-5 items-center">
                            <div
                                class="col-span-2 flex gap-2.5 ps-4 items-start text-base font-medium text-dark-primary">
                                <a href="{{ route('detail-breads', $cart->breads->id) }}"
                        ...
                        @if (isset($errorsPerBread[$cart->id]))
                            <div class="text-red-500 text-sm ps-4 mt-1">{{ $errorsPerBread[$cart->id] }}</div>
                        @endif
                    </div>
                </div>
            @endforeach
    ...
    <div class="hidden lg:block sticky bottom-0 bg-white w-full mt-6 rounded-lg p-5 transition-all duration-300"
        ...
        <div class="flex justify-between gap-5 items-center">
            <div class="text-sm sm:text-base text-dark-primary col-span-4 text-end">Jumlah Item ({{ count($carts) }})
            </div>
            <div class="flex items-center gap-4">
                <div class="text-sm sm:text-base text-dark-primary col-span-4 text-end">Total: <span
                        class="text-primary text-sm font-medium">Rp<span
